...
// TODO: Calculate the factorial of a number.
    ...
    public static void diceGame(int sides, String doRoll) {
        Scanner sc = new Scanner(System.in);
            Random rand = new Random();

    // included a 2nd method to get a random variable using the Random() method
    //        int upperbound = sides;
    //        int dice1 = rand.nextInt(upperbound);
    //        int dice2 = rand.nextInt(upperbound);

            int dice1 = (int) Math.floor(Math.random()*(sides) + 1);
            int dice2 = (int) Math.floor(Math.random()*(sides) + 1);
            ...
            if (doRoll.equals("n")) {
                ...
                //Recursion with method overloading to roll until base case is met for exiting the game.
                diceGame(sides, doRoll);
            }
    }
